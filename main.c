...
imu_acc a;
imu_g g;
float pitch=0,roll=0,pitch_filter=0,roll_filter=0;
float pitch_prev,roll_prev;

float roll_set=0.0;
float pitch_set=0.0;
float roll_pid=0.0,pitch_pid=0.0;
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
float pid(float kp,float kd,float ki,float angle,float angle_prev,float set_angle,float dt){
	float p = (set_angle-angle);
	static i_sum=0;
	p = p*kp;

	float d = (angle_prev-angle)/dt;
	d = d*kd;
	if(d>500)d=500;
	if(d<-500)d=-500;

	 i_sum += (set_angle-angle)*dt;
	float i = i_sum*ki;
	if(i>50.0)i=50.0;
	if(i<50.0)i=-50.0;
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  gx = g.g_x*dt*57.2958f;
	  gy = g.g_y*dt*57.2958f;

	  pitch_filter = 0.98*(pitch+gy) + 0.02*pitch_filter;
	  roll_filter = 0.98*(roll+gx) + 0.02*roll_filter;

	  //roll_pid = pid(roll_kp,roll_kd,roll_ki,roll_filter,roll_prev,roll_set,dt);
...
int _write(int file, char *ptr, int len)
{
    ...
    HAL_Delay(10); // small delay to let USB send
    return len;
}
...
void DWT_Init(void){
	...
	tclk = 1.0f/(float)SystemCoreClock;
}
uint32_t micros(){
